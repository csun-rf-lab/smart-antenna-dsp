...
MI4190 = serialport('/dev/ttyUSB0');                 % Linux Serial
...
verifyIfInPosition(MI4190,AZStartPos,POSITION_ERROR,0,measApp,0,0,'v');
...
measApp.writeConsoleLine(sprintf('\n[%s] Center frequency chosen: %.5f %s\n',datestr(now,'HH:MM:SS.FFF'),centerFreq,centerFreqUnits))
...
bootUSRPs(0,measApp); %Where is this function?
...
for currentDegree = degInterval
    ...
    measApp.writeConsoleLine(sprintf('[%s] Current Degree Measurement: %.2f',datestr(now,'HH:MM:SS.FFF'),currentDegree));
    
    AZInPosition = verifyIfInPosition(MI4190,currentDegree,POSITION_ERROR,loadBarProgress,measApp,anglesRemaining,incrementSize,'v');
    if (measApp.wantToStop) break; end
    ...
    if (AZIdle && AZInPosition)
        ...
        if (currentDegree ~= degInterval(end))
            ...
            measApp.writeConsoleLine(sprintf('\n[%s] Incrementing MI4190 Position by %.2f degrees',datestr(now,'HH:MM:SS.FFF'),incrementSize));
